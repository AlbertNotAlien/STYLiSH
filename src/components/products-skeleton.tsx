...
function createIndexArray(number: number) {
...
const ProductsSkeleton = forwardRef<HTMLDivElement, ProductsSkeletonProps>(
  ({ rowsLength = 1, columnsLength = 3 }, ref) => {
    const rows = createIndexArray(rowsLength);
    const columns = createIndexArray(columnsLength);

    return (
      <div ref={ref} className="flex flex-col gap-y-[50px]">
